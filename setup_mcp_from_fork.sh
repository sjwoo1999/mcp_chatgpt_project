...
echo "🧼 Step 5: Cleaning up backup directory..."
rm -rf openai-agents-python-backup

echo "✅ Step 6: Checking git status inside forked directory..."
cd openai-agents-python
git status

echo "💾 Step 7: Committing your custom changes..."
git add .
git commit -m '✨ Custom: Applied local MCP modifications'
git push origin main

echo "🔁 Step 8: Adding upstream for future updates..."
git remote remove upstream 2>/dev/null || true
git remote add upstream https://github.com/openai/openai-agents-python.git
git fetch upstream

echo "🏁 Done! Your fork is now synced, customized, and cleaned up."
